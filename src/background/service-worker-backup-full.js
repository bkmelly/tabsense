...
class TabSenseServiceWorker {
  ...
  async initialize() {
    ...
    try {
      ...
      try {
        ...
        // Already exists or failed - that's okay
        log('warn', 'Offscreen document creation:', error.message);
      }
      ...
      // Initialize AI Adapter (disabled - moved to offscreen)
      ...
      // Initialize APIs (disabled - moved to offscreen)
  ...
  /**
   * Send message to offscreen document for heavy processing
   */
  async sendToOffscreen(action, payload) {
    try {
      log('debug', `Sending to offscreen: ${action}`);
      ...
      log('error', 'Failed to send to offscreen:', error.message);
      throw error;
    }
  }
